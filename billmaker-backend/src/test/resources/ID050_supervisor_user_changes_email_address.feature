Feature: Supervisor changes email address
  ...
  Scenario Outline: Supervisor changes to a email address that has already been used (Error Flow)
    ...
    When I change the supervisor email "<Email>" to the new email address "<NewEmail>"
    Then the error message "Email already exists" is returned for change supervisor user email address
    Examples:
